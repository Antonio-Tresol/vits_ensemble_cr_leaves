...
class FolderScanner:
    ...
    def count_files(root_dir: str) -> dict:
        ...
        for folder in os.scandir(root_dir):
            if folder.is_dir():
                folder_counts[folder.name] = sum(1 for _ in os.scandir(folder) if _.is_file())
...
class ImageListBuilder:
    @staticmethod
    def build_list(root_dir: str, folders: List[str]) -> Tuple[List[str], List[int], List[int]]:
        """
        ...
        return images, labels, list(class_counts.values())
...
class ImageFolderDataset(Dataset):
    def __init__(self, root_dir: str, transform=None):
        ...
        self.images, self.labels, self.class_counts = ImageListBuilder.build_list(root_dir=self.root_dir, folders=self.folders)
